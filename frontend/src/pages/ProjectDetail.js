...
function ProjectDetail() {
  ...
  const startRankPolling = () => {
    ...
    if (rankPollingRef.current) return;
  ...
  const fetchProjectAndRankings = async () => {
    try {
      ...
      const rankingsRes = await axios.get(
        `${process.env.REACT_APP_SERVER_URL}/api/projects/${id}/saved-rankings`,
        {
          headers: {
            "Content-Type": "application/json",
          },
  ...
  const exportToCSV = () => {
    ...
    link.setAttribute(
      "download",
      `${project.websiteName}_keywords_${new Date().toISOString().split("T")[0]}.csv`,
    );
  ...
  const linkSheetToProject = async () => {
    try {
      ...
      const response = await axios.post(
        `${process.env.REACT_APP_SERVER_URL}/api/projects/${id}/link-sheet`,
        payload,
        {
          headers: {
            "Content-Type": "application/json",
  ...
  const updateSheetRanking = async () => {
    try {
      ...
      const response = await axios.get(
        `${process.env.REACT_APP_SERVER_URL}/api/projects/${id}/update-googlesheet`,
        {
          headers: {
            "Content-Type": "application/json",
          },
            ...
            <div className="detail-item">
              ...
              <span className="detail-value">
                {project.spreadsheet.spreadsheetTitle ? (
                    ...
                    >
                      <p>
                        ...
                        <span style={{ fontSize: "14px" }}>
                          {project.spreadsheet.sheets[0].tabName}
                        </span>
                      </p>
                      <Chip
                        variant="outlined"
                        ...
                        color="primary"
                        onClick={() => {
                          if (!isUpdateSheetRankingLoading)
                            updateSheetRanking();
                        }}
                  ...
                  project.spreadsheet.spreadsheetUrl != "" ? (
                  <Chip
                    variant="outlined"
                    label={
                      isLinkSheetLoading ? (
            ...
            <table>
              ...
              <tbody>
                <tr>
                  ...
                  <td>
                    {
                      project.rankings.filter(
                        (item) => item.ranking < 11 && item.ranking > 0,
                      ).length
                    }
                  </td>
                </tr>
                <tr>
                  ...
                  <td>
                    {
                      project.rankings.filter(
                        (item) => item.ranking < 21 && item.ranking > 10,
                      ).length
                    }
                  </td>
                </tr>
                <tr>
                  ...
                  <td>
                    {
                      project.rankings.filter(
                        (item) => item.ranking < 51 && item.ranking > 20,
                      ).length
                    }
                  </td>
                </tr>
                <tr>
                  ...
                  <td>
                    {
                      project.rankings.filter(
                        (item) => item.ranking <= 100 && item.ranking > 50,
                      ).length
                    }
                  </td>
                </tr>
                <tr>
                  ...
                  <td>
                    {
                      project.rankings.filter((item) => item.ranking === null)
                        .length
                    }
                  </td>
                </tr>
        ...
        <div className="detail-section">
          <div className="section-header">
            <div className="pdpDetailTabberHeaderWrapper">
              <ul>
                <li
                  className={tabberState === "keywords" ? "active" : ""}
                  onClick={() => setTabberState("keywords")}
                >
                  Keywords ({project.keywords.length})
                </li>
                <li
                  className={tabberState === "queries" ? "active" : ""}
                  onClick={() => setTabberState("queries")}
                >
                  Search Console
                </li>
              </ul>
          ...
          {rankingsLoading ? (
            ...
            <>
              {project.keywords.length > 0 && tabberState === "keywords" ? (
                      ...
                      {project.keywords.map((keyword, index) => {
                        ...
                        return (
                          <tr key={index}>
                            ...
                            <td
                              className={
                                rankingData.ranking ? "ranking-value" : ""
                              }
                              style={{ ...rankColorStatus, minWidth: "110px" }}
                            >
                              {rankingData.ranking === null
                                ? "Not in 100"
                                : rankingData.ranking === undefined
                                  ? "-"
                                  : rankingData.ranking}
                            </td>
                            <td
                            ...
                            >
                              {rankingData.previousRanking === null
                                ? "Not in 100"
                                : rankingData.previousRanking === undefined
                                  ? "-"
                                  : rankingData.previousRanking}
                            </td>
                            <td>
                ...
                tabberState === "keywords" ? (
                <p className="no-keywords">
                  No keywords added for this project.
                </p>
              ) : tabberState === "queries" ? (
                <QueryTable site={project.websiteUrl} />
              ) : (
              ...
              {rankings.length > 0 && tabberState === "keywords" && (
                <div className="rankings-info">
                  <small>
                    Rankings checked in {rankings[0]?.searchEngine || "Google"}{" "}
                    on{" "}
